    ...
    <script>
        ...
        function conseilFn() {
            return {
                ...
                deleteModal: false,
                conseil: blankConseil,
                editID: null,
                conseils: [],
                loading: false,
                onCreate() {
                    this.editID = null;
                    this.conseil = blankConseil
                    this.createModal = true
                },

                onEdit(info) {
                    this.editID = info.id;
                    this.conseil = {...info, maladies:info.maladies.map(x => x.id)}
                    this.createModal = true
                },
                ...
                async fetchinfo() {
                    ...
                    res = {data: {data: @json($conseils)}}
                    console.log(res);
                    this.conseils = res.data.data;
                    this.inited = true
                },
                async storeInfo() {
                    this.loading = true
                    res = await window.axios.post("/dashboard/conseils/", this.conseil)
                    console.log(res);
                    this.conseils.push(res.data.data)
                    this.conseil = blankConseil
                ...
                async deleteInfo() {
                    this.loading = true
                    res = await window.axios.delete(`/dashboard/conseils/${this.editID}`)
                    console.log(res);
                    this.conseils = this.conseils.filter(x => x.id !== this.editID)
                    this.conseil = blankConseil
                    this.editID = null;
                ...
                async updateInfo() {
                    this.loading = true
                    res = await window.axios.put(`/dashboard/conseils/${this.editID}`, this.conseil)
                    console.log(res);
                    let target = this.conseils.find(x => x.id == this.editID)
                    target.conseil = this.conseil
                    this.editID = null
                    this.conseil = blankConseil
                    this.createModal = false;
                    this.loading = false
...
@endsection
@section('main')
    <div x-data='conseilFn()' x-init="fetchinfo()">
        <div x-show="createModal" x-transition:enter="transition duration-200 transform ease-out"
            ...
            <div class="relative z-40 p-8 px-16 bg-white rounded-lg shadow-xl" @click.stop>
                ...
                <p class="text-3xl font-bold text-center" x-text="editID ? 'Modification' : 'Nouvelle conseil'"></p>

                <div class="mt-4 mb-6 -mx-3">
                    <div class="px-3 md:w-full">
                        <label class="block mb-2 text-xs font-bold tracking-wide uppercase text-grey-darker" for="title">
                            Titre du conseil
                        </label>
                        <input
                            class="block w-full px-4 py-3 mb-3 border rounded appearance-none bg-grey-lighter text-grey-darker border-grey-lighter"
                            id="title" type="text" name="title" x-model="conseil.title" />

                    </div>

                    <div class="px-3 md:w-full">
                        <label class="block mb-2 text-xs font-bold tracking-wide uppercase text-grey-darker" for="content">
                            Contenu
                        </label>
                        <textarea
                            class="block w-full px-4 py-3 mb-3 border rounded appearance-none bg-grey-lighter text-grey-darker border-grey-lighter"
                            id="content" type="text" name="content" x-model="conseil.content"></textarea>
                    ...
                    <div class="px-3 md:w-full">
                        ...
                        <select
                            class="block w-full px-4 py-3 mb-3 border rounded appearance-none bg-grey-lighter text-grey-darker border-grey-lighter"
                            id="content" type="text" name="content" multiple x-model="conseil.maladies">
                ...
                <div class="flex justify-center mt-4">
                    <button @click='loading ? false : editID ? updateInfo(): storeInfo(conseil)'
        ...
        <div x-show="deleteModal" x-transition:enter="transition duration-200 transform ease-out"
            ...
            <div class="relative z-40 p-8 px-16 bg-white rounded-lg shadow-xl" @click.stop>
                <button @click='deleteModal = false'
                    class="absolute w-6 h-6 text-sm text-white transition-colors rounded-full -top-2 -right-2 bg-primary hover:bg-primary"><i
                        class="fa fa-window-close" aria-hidden="true"></i></button>
                <p class="text-3xl font-bold text-center">Voulez vous vraiment supprimer ce conseil?</p>
                <p class="mt-4 text-lg" x-text="conseil.name"></p>
                <div class="flex justify-center mt-4">
            ...
            <div class="grid grid-cols-12 col-span-12 gap-6 xxl:col-span-9">
                <div class="col-span-12 mt-8">
                    <div class="flex items-center h-10 intro-y">
                        <h2 class="mr-5 text-lg font-medium truncate">Conseils</h2>
                    </div>
                                ...
                                <div class="flex-1 w-full ml-2">
                                    <div>
                                        <div class="mt-3 text-3xl font-bold leading-8"
                                            x-text="inited ? conseils.length :'Veillez patienter'"></div>

                                        <div class="mt-1 text-base text-gray-600">Conseils Ajoutées</div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    ...
                    <div class="grid grid-cols-1 gap-2 lg:grid-cols-1">
                        <div class="p-4 bg-white rounded-lg shadow-lg">
                            ...
                            <div class="flex justify-between">
                                <h1 class="text-base font-bold">Liste des conseils</h1> <template x-if='!inited'>
                                            ...
                                            <div
                                                ...
                                                <table class="min-w-full divide-y divide-gray-200">
                                                    ...
                                                    <tbody class="bg-white divide-y divide-gray-200">
                                                        ...
                                                        <template x-for="info in conseils" :key="info.id">
                                                            <tr>
